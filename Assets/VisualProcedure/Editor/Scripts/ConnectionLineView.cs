...
namespace VisualProcedure.Editor.Scripts
{
    ...
    public class ConnectionLineView
    {
        ...
        public ConnectionLineView(PortEditorView portViewA, PortEditorView portViewB, GraphEditorData graphEditorData)
        {
            ...
            FlowOutPortView = portViewA.FlowType == FlowType.In ? portViewB : portViewA;
            FlowInPortView = portViewB.FlowType == FlowType.In ? portViewB : portViewA;

            //每个流出端口,只允许连接一个
            graphEditorData.ClearPortAllConnections(FlowOutPortView);
        ...
        public static bool CheckPortsCanLine(PortEditorView portA, PortEditorView portB)
        {
            ...
            var flowOutPortView = portA.FlowType == FlowType.Out ? portA : portB;
            var flowInPortView = portA.FlowType == FlowType.In ? portA : portB;
        ...
        public void DrawDragLine()
        {
            ...
            if (draggingPort.FlowType == FlowType.Out)
            {
                if (startPos.x >= endPos.x)
                {
                    ...
                    var horizontalDistance = startPos.x - endPos.x;
                ...
                else
                {
                    ...
                    var horizontalDistance = endPos.x - startPos.x;
        ...
        {
            return HandleUtility.DistancePointBezier(positionInWindow, startPos, endPos, startTangent, endTangent) <
                   10f;
        }
