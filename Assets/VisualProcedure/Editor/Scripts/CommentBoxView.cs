    ...
    public class CommentBoxView
    {
        ...
        public CommentBoxView(GraphEditorWindow graph, Vector2 startPositionInGraph, Vector2 endPositionInGraph)
        {
            ...
            rectInGraph = new Rect(startPositionInGraph,
                new Vector2(endPositionInGraph.x - startPositionInGraph.x,
                    endPositionInGraph.y - startPositionInGraph.y));
        ...
        public static void DrawDraggingCommentBox(GraphEditorWindow graph,
                                                  ...
                                                  Vector2 endPositionInGraph)
        {
            var startPositionInWindow = graph.GraphPositionToWindowPosition(startPositionInGraph);
            var endPositionInWindow = graph.GraphPositionToWindowPosition(endPositionInGraph);

            var drawRect = new Rect(startPositionInWindow,
                new Vector2(endPositionInWindow.x - startPositionInWindow.x,
                    endPositionInWindow.y - startPositionInWindow.y));
        ...
        public void Draw()
        {
            var startPositionInWindow = graph.GraphPositionToWindowPosition(rectInGraph.position);
            var endPositionInWindow = graph.GraphPositionToWindowPosition(rectInGraph.max);
        ...
        private void UpdateHeadRect()
        {
            ...
            var commentTextHeight = Utility.GetGuiStyle("CommentText")
                                           .CalcHeight(new GUIContent(comment), rectInWindow.width);
        ...
        {
            var result = BoxEdge.None;

            const float distance = 10f;
            //check right
            if (Mathf.Abs(rectInWindow.xMax - positionInWindow.x) <= distance &&
                positionInWindow.y <= rectInWindow.yMax && positionInWindow.y >= rectInWindow.yMin)
            {
                result = BoxEdge.Right;
            }
            //check bottom
            else if (Mathf.Abs(rectInWindow.yMax - positionInWindow.y) <= distance &&
                     positionInWindow.x <= rectInWindow.xMax && positionInWindow.x >= rectInWindow.xMin)
            {
                result = BoxEdge.Bottom;
            }
            //check top
            else if (Mathf.Abs(rectInWindow.yMin - positionInWindow.y) <= distance &&
                     positionInWindow.x <= rectInWindow.xMax && positionInWindow.x >= rectInWindow.xMin)
            {
                result = BoxEdge.Top;
            }
            else if (Mathf.Abs(rectInWindow.xMin - positionInWindow.x) <= distance &&
                     positionInWindow.y <= rectInWindow.yMax && positionInWindow.y >= rectInWindow.yMin)
            {
                result = BoxEdge.Left;
            }
        ...
        public void Drag(List<NodeEditorView> nodeViewList, Vector2 dragOffset)
        {
            ...
            if (nodeViewList != null && nodeViewList.Count > 0)
            {
                ...
                {
                    var nodeEditorView = nodeViewList[i];
