...
namespace Game.GameEvent
{
    ...
    public sealed partial class EventPool<T> where T : BaseEventArgs
    {
        ...
        public void Subscribe(int id, EventHandler<T> handler)
        {
            ...
            else if ((m_EventPoolMode & EventPoolMode.AllowMultiHandler) == 0)
            {
                throw new GameFrameworkException(
                    GameFramework.Utility.Text.Format("Event '{0}' not allow multi handler.", id.ToString()));
            }
            else if ((m_EventPoolMode & EventPoolMode.AllowDuplicateHandler) == 0 && Check(id, handler))
            {
                throw new GameFrameworkException(
                    GameFramework.Utility.Text.Format("Event '{0}' not allow duplicate handler.", id.ToString()));
            }
            else
            {
                m_EventHandlers.Add(id, handler);
        ...
        public void Unsubscribe(int id, EventHandler<T> handler)
        {
            ...
            {
                throw new GameFrameworkException(
                    GameFramework.Utility.Text.Format("Event '{0}' not exists specified handler.", id.ToString()));
            }
        }
        ...
        private void HandleEvent(object sender, T e)
        {
            ...
            {
                throw new GameFrameworkException(GameFramework.Utility.Text.Format("Event '{0}' not allow no handler.",
                    e.Id.ToString()));
            }
        }
    }
